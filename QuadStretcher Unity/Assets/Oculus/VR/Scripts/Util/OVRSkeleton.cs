...
public class OVRSkeleton : MonoBehaviour
{
    ...
    //////CUSTOM EDITTED START//////
    [SerializeField]
    private bool CustomBones = false;
	//////CUSTOM EDITTED END//////
	[SerializeField]
	...
	[SerializeField]
	public bool _updateRootPose = false;
	[SerializeField]
	...
	private void Start()
	{
        if(CustomBones)
        {
            _bonesGO = transform.Find("Bones").gameObject;
            _bindPosesGO = transform.Find("BindPoses").gameObject;
            Initialize();
        }
		if (ShouldInitialize())
		{
	...
	protected virtual void InitializeBones()
	{
        ...
        for (int i = 0; i < _bones.Count; ++i)
        {
            ...
            if (CustomBones)
            {
                bone.Transform = RecursiveFindChild(_bonesGO.transform, bone.Id.ToString());
            }
            else
            {
	...
	private void InitializeBindPose()
	{
		...
		for (int i = 0; i < _bindPoses.Count; ++i)
		{
            ...
            if (CustomBones)
            {
                bindPoseBone.Transform = RecursiveFindChild(_bindPosesGO.transform, bindPoseBone.Id.ToString());
            }
            else
            {
	...
	{
#if UNITY_EDITOR
		if (ShouldInitialize() || CustomBones)
		{
			Initialize();
		}
#endif
